...
export const name1_REGEX = /^[a-zA-Z. ]+$/;
export const name2_REGEX = /^[a-zA-Z\s\(\)\[\]]+$/;
...
export const ValidateEmailOrPhone = (input) => {
  if (input !== '') {
    if (EMAILREGEX.test(input)) {
      ...
      return ErrorMessages.InvalidEmailOrPhone;  // Invalid email/phone
    }
  } else {
    return ErrorMessages.InputEmpty;  // Empty input
  }
};
...
export const ValidateFirmemployername = (firm) => {
  if (firm !== '') {
    if (firm.length >= 2 && name1_REGEX.test(firm)) {
      return '';
    } else {
      if (firm.length < 2) {
...
export const ValidateDesignation = (design) => {
  if (design !== '') {
    if (design.length >= 2 && name2_REGEX.test(design)) {
      return '';
    } else {
      if (design.length < 2) {
