...
const Form4 = (props) => {
    ...
    const submitData = async () => {
        ...
        try {
            // Combine and format family members' data
            const familyMembersData = detailsArray.concat(detailsArray1).map(details => ({
                full_name: details?.FName || details?.FName1 || '',
                dob: moment(details?.birthDate || details?.birthDate1).format('YYYY-MM-DD') || '',
                business_or_service: details?.Business || details?.Business1 || '',
                firm_or_employer: details?.employer || details?.employer1 || '',
                mobile_no: details?.phone1 || details?.phone11 || '',
                relation: details?.Relation1 || details?.Relation || ''
            }));
    
            // Preparing data in x-www-form-urlencoded format
            const params = new URLSearchParams();
            params.append('first_name', formData1?.FName || '');
            params.append('middle_name', formData1?.MName || '');
            params.append('surname', formData1?.SName || '');
            params.append('gender', formData1?.Gender || '');
            params.append('dob', moment(formData1?.birthDate).format('YYYY-MM-DD') || '');
            params.append('sub_caste', formData1?.Subcast || '');
            params.append('address_line1', formData2?.Address || '');
            params.append('address_line2', formData2?.Address1 || '');
            params.append('city', formData2?.City || '');
            params.append('taluka', formData2?.Taluka || '');
            params.append('district', formData2?.District || '');
            params.append('state', formData2?.State || '');
            params.append('pincode', formData2?.Pincode || '');
            params.append('business_or_service', formData3?.Business2 || '');
            params.append('firm_or_employer', formData3?.employer || '');
            params.append('designation', formData3?.organisation || '');
            params.append('mobile1', phone || '');
            params.append('mobile2', Mobile1 || '');
            params.append('email_address', Email || '');
            params.append('no_of_family_members_male', Male || 0);
            params.append('no_of_family_members_female', Female || 0);
    
            // Add family members' data in URL-encoded format
            familyMembersData.forEach((member, index) => {
                params.append(`family_members[${index}][full_name]`, member.full_name);
                params.append(`family_members[${index}][dob]`, member.dob);
                params.append(`family_members[${index}][business_or_service]`, member.business_or_service);
                params.append(`family_members[${index}][firm_or_employer]`, member.firm_or_employer);
                params.append(`family_members[${index}][mobile_no]`, member.mobile_no);
                params.append(`family_members[${index}][relation]`, member.relation);
            });
    
            // Debugging: Log the data to be sent
            console.log('Data being submitted:', params.toString());
    
            // Sending the request to the backend
            const response = await fetch('https://zeelogic.in/census/save_data.php', {
                method: 'POST',
                headers: {
    ...
    //             "no_of_family_members_male:", Male,
    //             "no_of_family_members_female:", Female,
    //             "Family Members Data222222222222:", familyMembersData,
    ...
    //                 "dafagfjsdhgkfjdhl;gfjlghfgggggggh444444444444444444444444444444444444"
        ...
        <KeyboardAwareScrollView
            ...
            <View style={styles.viewStyle}>
                ...
                <Text style={styles.firstname}>
                    Mobile-1   <Text style={styles.starStyle}>*</Text>
                </Text>
                ...
                <Text style={styles.firstname}>
                    Mobile-2 (Whatsapp) <Text style={styles.starStyle}>*</Text>
                    ...
                    <>
                        {detailsArray.map((details, index) => (
                            <View key={index} style={{
                                ...
                                {/* Mobile Number */}
                                <Text style={styles.firstname}>Mobile Number <Text style={styles.starStyle}>*</Text></Text>
                                <Inputfield
                    ...
                    <>
                        {detailsArray1.map((details1, index) => (
                            <View key={index} style={{
                                ...
                                />
                                <Text style={styles.firstname}>
                                    Mobile Number <Text style={styles.starStyle}>*</Text>
                                </Text>
                                <Inputfield
                                    // edit={EmailEdit}
                                    placeholder={'Enter Mobile Number'}
